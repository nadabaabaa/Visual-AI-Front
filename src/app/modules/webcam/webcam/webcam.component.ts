...
export class WebcamComponent implements AfterViewInit, OnDestroy {
  ...
  openCamera() {
    // Assurez-vous que la caméra n'est pas déjà initialisée
    if (!this.videoInitialized) {
      this.detectAvailableDevices()
        .then(() => {
          // Démarrer la caméra lorsque l'utilisateur clique sur le bouton
          this.switchToVideoInput(null);
        })
        .catch((err: string) => {
          this.initError.next(<WebcamInitError>{message: err});
          // Fallback : essayez quand même de charger la webcam, même si l'énumération des appareils a échoué
          this.switchToVideoInput(null);
        });
    }
  }
